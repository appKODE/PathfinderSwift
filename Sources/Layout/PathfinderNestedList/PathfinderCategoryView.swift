...
final class PathfinderCategoryView: UIView {
    // Handlers
    var onSelectQuery: PathfinderNestedListView.QuerySelectionHandler?
    ...
    private func fillQueriesStackView() {
        ...
        queryGroupConfig.enumerated().forEach { index, query in
            let queryView = PathfinderQueryView(config: query, isLast: (index + 1) == queryGroupConfig.count)
