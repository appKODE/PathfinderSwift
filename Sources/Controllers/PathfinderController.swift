...
final class PathfinderController: UIViewController {
    ...
    private let presentationStyleSelectorView = PathfinderPresentationStyleSelectorView(.list)

    private var presentationPagerContainer = UIStackView()
    private let serverSelector = ServerSelectorView()
    private var nestedView = PathfinderNestedListView(Pathfinder.shared.getGroupedRequests())
    ...
    private func refreshData() {
        nestedView = PathfinderNestedListView(Pathfinder.shared.getGroupedRequests())
        dataList   = Pathfinder.shared.getAllUrls()

        nestedView.onSelectQuery = { [weak self] query in
            self?.present(QueryEditorController(config: query), animated: true)
        }
    ...
    private func handlePresentationStyleChange(to style: PathfinderQueriesPresentationStyle) {
        switch style {
        case .list:
...
extension PathfinderController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let controller = QueryEditorController(config: queryModel)
